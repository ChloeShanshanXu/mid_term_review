## Code reference Eric Charnesky:
## https://github.com/EricCharnesky/CIS2001-Winter2022/blob/main/Lab5/main.py
...
class TransportMethod:
    def __init__(self, num_of_items_till_full):
        self._num_of_items_till_full = num_of_items_till_full
        self._current_item_count = 0
        self._time_till_fully_loaded = 0

    def load_item(self, time_worker_arrived):
        self._current_item_count += 1
        if self._current_item_count == self._num_of_items_till_full:
            self._time_till_fully_loaded = time_worker_arrived

    def get_time_till_fully_loaded(self):
        return self._time_till_fully_loaded
...
class Dock:
    ...
    def __init__(self, item_per_train, item_per_plane, train_items, plane_items):
        self._train = [None]  # index 0 is none
        for value in item_per_train.split():
            self._train.append(TransportMethod(int(value)))

        self._plane = [None]
        for value in item_per_plane.split():
            self._plane.append(TransportMethod(int(value)))
    ...
    def load_trains(self):
        ...
        while not self._train_items.is_empty():
            ...
            while not current_stack.is_empty():
                ...
                self._train[current_item].load_item(arrival_time)
    ...
    def load_planes(self):
         ...
         while not self._plane_items.is_empty():
             ...
             self._plane[current_item].load_item(arrival_time)
             current_time += current_item * 2 * self.PLANE_ONE_WAY_TIME

    def get_train_fully_loaded_times(self):
        return " ".join(str(train.get_time_till_fully_loaded()) for train in self._train[1:])

    def get_plane_fully_loaded_times(self):
        return " ".join(str(plane.get_time_till_fully_loaded()) for plane in self._plane[1:])
